...
class persona extends Validations{
	...
	*/
	...
	ADD_contrasena_persona_validation(){
		
		if (!(this.min_size('contrasena_persona',8))){
			this.dom.mostrar_error_campo('contrasena_persona','contrasena_persona_min_size_KO');
			return "contrasena_persona_min_size_KO";
		}
		if (!(this.max_size('contrasena_persona',12))){
			this.dom.mostrar_error_campo('contrasena_persona','contrasena_persona_max_size_KO');
			return "contrasena_persona_max_size_KO";
		}
		// allowed format aA to zZ letter
		if (!(this.format('contrasena_persona', '^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9]).*$'))){
			this.dom.mostrar_error_campo('contrasena_persona','contrasena_persona_format_KO');
			return "contrasena_persona_format_KO";
		}
		this.dom.mostrar_exito_campo('contrasena_persona');
		return true;
	}
	...
	*/
	...
	ADD_submit_persona(){
		...
		let result = (
					(this.ADD_contrasena_persona_validation()) === true &&
					(this.ADD_nombre_persona_validation()) === true
					)
					document.getElementById('nombre_persona').focus();
					console.log(result)
		return result;	
